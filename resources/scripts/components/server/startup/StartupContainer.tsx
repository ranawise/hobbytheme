...
const StartupContainer = () => {
    ...
    return !data ? (
        ...
        <ServerContentBlock title={'Startup Settings'} showFlashKey={'startup:image'} compact>
            ...
            <div css={tw`md:flex`}>
                <TitledGreyBox title={'Startup Command'} css={tw`flex-1`}>
                    <div css={tw`px-1 py-2`}>
                        <p css={tw`font-mono bg-white/5 border border-white/10 rounded-lg py-2 px-4 text-neutral-200`}>
                            {data.invocation}
                        </p>
                    </div>
                </TitledGreyBox>
                    ...
                    {Object.keys(data.dockerImages).length > 1 && !isCustomImage ? (
                        <>
                            <InputSpinner visible={loading}>
                                <Select
                                    css={tw`!bg-surface !border-white/10 !text-neutral-200 focus:!border-primary-600/60`}
            ...
            <h3 css={tw`mt-8 mb-2 text-2xl text-white font-semibold`}>Variables</h3>
