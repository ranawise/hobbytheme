...
const TaskDetailsModal = ({ schedule, task }: Props) => {
    ...
    return (
        ...
        >
            {({ isSubmitting, values }) => (
                <ThemedForm css={tw`m-0`}>
                    <FlashMessageRender byKey={'schedule:task'} css={tw`mb-4`} />
                    <h2 css={tw`text-2xl mb-6 text-white font-semibold`}>{task ? 'Edit Task' : 'Create Task'}</h2>
                    <div css={tw`flex`}>
                    ...
                    <div css={tw`mt-6 bg-surface border border-white/10 p-4 rounded-lg`}>
                        <FormikSwitch
