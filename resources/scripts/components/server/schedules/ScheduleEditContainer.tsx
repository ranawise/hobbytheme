...
const CronBox = ({ title, value }: { title: string; value: string }) => (
    <div css={tw`bg-surface border border-white/10 rounded-lg p-3`}>
...
const ActivePill = ({ active }: { active: boolean }) => (
    <span
        css={[
            tw`rounded-full px-2 py-px text-xs ml-4 uppercase font-medium`,
            active ? tw`bg-green-500/10 text-green-400` : tw`bg-neutral-500/10 text-neutral-400`,
        ]}
    >
...
export default () => {
    ...
    return (
        <PageContentBlock title={'Schedules'} compact>
            <div
            ...
            >
                ...
                {!schedule || isLoading ? (
                    ...
                    <>
                        ...
                        <ScheduleCronRow
                            cron={schedule.cron}
                            css={tw`sm:hidden bg-surface border border-white/10 rounded-lg mb-4 p-3`}
                        />
                        <div>
                            <div
                                css={tw`sm:flex items-center bg-surface border border-white/10 p-3 sm:p-6 rounded-lg`}
                            >
                                <div css={tw`flex-1`}>
                                    <h3 css={tw`flex items-center text-white text-2xl font-semibold`}>
                                        {schedule.name}
                                        {schedule.isProcessing ? (
                                            <span
                                                css={tw`flex items-center rounded-full px-2 py-px text-xs ml-4 uppercase font-medium bg-yellow-500/10 text-yellow-400`}
                                            >
                            ...
                            <div css={tw`bg-surface border border-white/10 rounded-lg mt-4 overflow-hidden`}>
