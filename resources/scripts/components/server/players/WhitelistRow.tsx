...
export default ({ username, note, onNoteChange, onRemoved }: Props) => {
    ...
    const remove = () => {
        getFileContents(uuid, '/whitelist.json')
            .then((raw) => {
                ...
                return saveFileContents(uuid, '/whitelist.json', JSON.stringify(updated, null, 2));
            })
    ...
    return (
        <Row>
            <Dialog.Confirm
                open={confirmOpen}
                title={`Remove ${username} from whitelist?`}
                confirm={'Remove'}
                onClose={() => setConfirmOpen(false)}
                onConfirmed={remove}
            >
                <strong>{username}</strong> will no longer be able to join the server.
            </Dialog.Confirm>
            <MinecraftAvatar username={username} size={36} />
            <div css={tw`flex-1 min-w-0`}>
                <p css={tw`text-sm font-medium text-neutral-200 truncate`}>{username}</p>
                <p css={tw`text-xs text-neutral-500 mt-0.5`}>Whitelisted</p>
            </div>
