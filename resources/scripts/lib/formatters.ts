...
/**
 * Returns the address players should use to connect to a server. Prefers the
 * server's provisioned domain(s) (e.g. "steve.hobbyservers.com") over the raw
 * allocation IP, since a domain is friendlier and survives the underlying
 * node's IP changing. Falls back to the allocation when no domain has
 * provisioned yet (or never will, e.g. an egg without DNS support).
 */
function connectAddress(
    domains: string[],
    allocation?: { alias: string | null; ip: string; port: number }
): string {
    if (domains.length > 0) {
        return domains.join(' or ');
    }

    return allocation ? `${allocation.alias || ip(allocation.ip)}:${allocation.port}` : 'n/a';
}

export { ip, mbToBytes, bytesToString, connectAddress };
